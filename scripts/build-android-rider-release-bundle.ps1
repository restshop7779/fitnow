...
'@
  ...
  $Bundle = Get-ChildItem "app\build\outputs\bundle\release\*.aab" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
  $StableRiderBundle = Join-Path $DistApkDir "fitnow-rider-release.aab"
  New-Item -ItemType Directory -Path $DistApkDir -Force | Out-Null
  Copy-Item -Path $Bundle.FullName -Destination $StableRiderBundle -Force
  Write-Host "FitNow Rider release bundle: $($Bundle.FullName)"
  Write-Host "FitNow Rider stable release bundle: $StableRiderBundle"
