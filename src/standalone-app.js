...
import {
  applyStoredOrderStatus as applyStoredOrderStatusFromStore,
  labelFromStep,
  mergeOrderLists as mergeStoredOrderLists,
  paymentLabelFromStep,
  readOrderStatusStore,
  saveOrderStatusOverride,
  statusFromStep,
  stepFromStatus,
} from "./standalone/orderStatus.js";
import {
...
import {
  adminPreReleaseManualActions,
  adminPreReleaseQuickActions,
  adminPreReleaseReportText as buildAdminPreReleaseReportText,
  adminQaChecklistItemKey,
  adminQaChecklistLabelByKey,
  adminQaChecklistProgress,
  adminQaChecklistRemainingRows,
  adminQaChecklistReportRows,
  adminQaChecklistReportText as buildAdminQaChecklistReportText,
  adminQaChecklistSections,
  clearAdminQaChecklistStore,
  qaScenarioActionLabel,
  qaScenarioActionManualItems,
  qaScenarioActionSuccessUpdates,
  readAdminQaChecklistStore,
  updateAdminQaChecklistStore,
} from "./standalone/adminQa.js";
import {
  assignedRiderLabel,
  defaultPartnerRate,
  deliveryPartnerForOrder,
  deliveryPartnerServesOrder,
  deliveryShortcutItems as buildDeliveryShortcutItems,
  ...
  riderSettlementRate,
  riderWorkBadge,
  riderWorkRows as buildRiderWorkRows,
...
import {
  ...
  adminOrderGroupedListMarkup,
  adminOrderListRowData,
...
import {
  ...
  returnRefundWindowLabel,
} from "./standalone/refunds.js";
...
import realFitModelImage from "../assets/fitnow-real-fit-model.png";
      ...
      function updateRiderNickname(partnerName, index, value) {
        ...
        renameRiderNickname(partner, index, value);
      ...
      function applyStoredOrderStatus(order) {
        return applyStoredOrderStatusFromStore(order, { applyStoredSettlementStatus });
      }

      function mergeOrderLists(primary, secondary) {
        return mergeStoredOrderLists(primary, secondary, applyStoredOrderStatus);
      }
      ...
      function renderDeliveryRiderGroups(orders, emptyText, badge, actionLabel, actionStep) {
        return deliveryRiderGroupsMarkup(orders, emptyText, badge, actionLabel, actionStep, {
          assignedRiderLabel,
      ...
      function renderDeliveryWarnings(items) {
        return deliveryWarningsMarkup(items, { assignedRiderLabel });
      }
      ...
      function setAdminQaChecklistItem(key, checked) {
        const { store, progress } = updateAdminQaChecklistStore({ [key]: !!checked });
      ...
      function markAdminQaChecklistItems(updates = {}, options = {}) {
        ...
        const { store, progress } = updateAdminQaChecklistStore(updates);
      ...
      function adminQaChecklistReportText() {
        return buildAdminQaChecklistReportText(readAdminQaChecklistStore(), { timeLabel: testToolTimeLabel });
      }
      ...
      function adminPreReleaseReportData() {
        ...
        const quickActions = adminPreReleaseQuickActions(qaStore, diagnostic, testMeta, { isToday: isTodayTestToolTime });
      ...
      function adminPreReleaseReportText() {
        return buildAdminPreReleaseReportText(adminPreReleaseReportData(), { timeLabel: testToolTimeLabel });
      }
      ...
      function markQaScenarioActionSuccess(action) {
        const updates = qaScenarioActionSuccessUpdates(action);
        markAdminQaChecklistItems(updates);
        return Object.keys(updates);
      }

      async function runQaScenarioAction(action) {
        const label = qaScenarioActionLabel(action);
      ...
      function riderWorkRows(orders) {
        const visiblePartners = currentAdmin && currentAdmin.role === "delivery"
          ? deliveryPartners.filter((partner) => partner.name === currentAdmin.name)
          : deliveryPartners;
        return buildRiderWorkRows(orders, visiblePartners);
      }
      ...
      function renderRiderWorkBoard(orders) {
        ...
        return riderWorkBoardMarkup(rows, { formatKRW });
      }
      ...
      function renderRiderNicknameManager() {
        const visiblePartners = currentAdmin && currentAdmin.role === "delivery"
          ? deliveryPartners.filter((partner) => partner.name === currentAdmin.name)
          : deliveryPartners;
        return riderNicknameManagerMarkup(visiblePartners);
      }
      ...
      function renderAdminModeBanner(orders = []) {
        ...
        node.innerHTML = adminModeBannerMarkup(diagnostic, retentionLabel);
      }
      ...
      function renderAdminReleaseReadiness(orders = []) {
        ...
        node.innerHTML = adminReleaseReadinessMarkup({ diagnostic, items, readyCount, allReady, settlementExportCount });
      }

      function deliveryShortcutItems(orders = []) {
        return buildDeliveryShortcutItems(orders, { isDeliveryOrderClaimed, isTodayOrder });
      }
      ...
      function renderDeliveryWorkShortcuts(orders = []) {
        ...
        node.innerHTML = deliveryWorkShortcutsMarkup(items, currentAdmin.name);
      }
      ...
      function renderAdminReviewModeration() {
        ...
        summary.innerHTML = adminReviewModerationSummaryMarkup(reviews);
        list.innerHTML = adminReviewModerationListMarkup(reviews, {
          renderReviewPhoto,
          reviewModerationKey,
        });
      }
      ...
      async function renderAdminOrders(ordersOverride) {
        ...
        const orderRows = visibleOrders.map((order) => adminOrderListRowData(order, {
          assignedRiderLabel,
          adminOrderBadge,
          cancelReasonLabel,
          formatKRW,
        ...
        list.innerHTML = adminOrderGroupedListMarkup(orderRows, {
